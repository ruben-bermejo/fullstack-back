...
def init_database():
    ...
    if not productos:
        ...
        for articulo in articulos:
            if articulo['disponible'] == 1 and articulo['stock'] > 0:
                solicitadas = min(articulo['stock'], 2)
                put(url = url_articulo + '/'
                            + str(articulo['articulo_id'])
                            + '/salida_stock',
                        timeout = 60,
                        headers = config_data['almacen']['api_key'],
...
def solicitar_producto(id_producto):
    ...
    if cantidad > 0:
        ...
        url_articulo = config_data['almacen']['base_url'] + '/articulo/' + art_id
        response_data = get(url = url_articulo,
                        timeout = 60,
                        headers = config_data['almacen']['api_key'])
        ...
        put(url = url_articulo + '/salida_stock',
                timeout = 60,
